...
import utils
from BB.models.Logistic_explainer import Logistic_Regression
...
from Explainer.profiler import fit_g_cub_profiler, fit_residual_cub_profiler
...
def setup_init_params(args, root, seed):
    ...
    g_tb_logs_path = None
    optimizer = None
    if args.logistic_explainer == "n":
        g_chk_pt_path = os.path.join(
            args.checkpoints, args.dataset, "Baseline_PostHoc", args.arch, root
        )
        g_output_path = os.path.join(
            args.output, args.dataset, "Baseline_PostHoc", args.arch, root
        )
        g_tb_logs_path = os.path.join(
            args.logs, args.dataset, "Baseline_PostHoc", args.arch, root
        )
        ...
        optimizer = torch.optim.SGD(model.explainer.parameters(), lr=0.01, momentum=0.9, weight_decay=5e-4)
    elif args.logistic_explainer == "y":
        g_chk_pt_path = os.path.join(
            args.checkpoints, args.dataset, "Baseline_PostHoc", args.arch, "Logistic_explainer"
        )
        g_output_path = os.path.join(
            args.output, args.dataset, "Baseline_PostHoc", args.arch, "Logistic_explainer"
        )
        g_tb_logs_path = os.path.join(
            args.logs, args.dataset, "Baseline_PostHoc", args.arch, "Logistic_explainer"
        )
        model = Logistic_Regression(
            len(args.concept_names), len(args.labels)
        ).to(device)
    ...
    args.g_output_path = g_output_path
    args.g_chk_pt_path = g_chk_pt_path
    ...
    print("############# Paths ############# ")
    return model, optimizer, g_chk_pt_path, g_tb_logs_path, g_output_path
...
def test_glt(args):
    ...
    # test_transform = transforms["save_transform"]
    ...
    for seed in range(args.seed):
        ...
        if args.expert_to_train == "explainer":
            ...
            test_explainer(args, seed, cov, lr_explainer, root, iteration, use_concepts_as_pi_input, train_loader,
                           val_loader,
                           test_loader)
        elif args.expert_to_train == "residual":
            test_residual(args, seed, root, iteration, use_concepts_as_pi_input, train_loader, val_loader, test_loader)


def test_residual(args, seed, root, iteration, use_concepts_as_pi_input, train_loader, val_loader, test_loader):
...
def test_explainer(
        args, seed, cov, lr_explainer, root, iteration, use_concepts_as_pi_input, train_loader, val_loader, test_loader
):
...
def train_glt(args):
    ...
    val_dataset = Dataset_cub_for_explainer(
        dataset_path, "val_proba_concepts.pt", "val_class_labels.pt", "val_attributes.pt", val_data, val_transform
    )
