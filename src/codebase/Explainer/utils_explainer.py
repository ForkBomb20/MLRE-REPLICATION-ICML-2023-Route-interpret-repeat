...
def get_details_cubs_experiment(i, arch, alpha_KD, temperature_lens, layer):
    ...
    if i == 1:
        pickle_in = open(
            os.path.join(
                f"./out/cub/explainer/{arch}/lr_{lr}_epochs_500_temperature-lens_{temperature_lens}_use-concepts-as-pi-input_True_input-size-pi_2048_cov_{cov}_alpha_0.5_selection-threshold_0.5_lambda-lens_0.0001_alpha-KD_{alpha_KD}_temperature-KD_10.0_hidden-layers_1_layer_{layer}_explainer_init_none",
...
def get_data_loaders_per_iter_completeness(args):
    if args.dataset == "cub" and (args.arch == "ResNet50" or args.arch == "ResNet101"):
        ...
        dataset_path = f"./out/{args.dataset}/completeness/{args.arch}/dataset"
...
def get_test_loaders_per_iter_completeness(i, args):
    ...
    if args.dataset == "cub":
        ...
        dataset_path = os.path.join(
            f"./out/{args.dataset}/explainer/{args.arch}/lr_{lr}_"
...
def get_data_tuple(tuple, dataset, device, per_iter_completeness=False):
    ...
    elif dataset == "cub" and per_iter_completeness:
